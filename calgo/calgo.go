// calgo.go, derived from calg.go, from caltcell.go, which was derived from calgo.go.  This version uses colortext.
...
var docNames []string // to be populated by the conf or ini file
...
func main() {
	...
	if *helpflag || HelpFlag {
		...
		fmt.Println(" Looks for calgo.conf or calgo.ini.")
		fmt.Println()
		flag.PrintDefaults()
		os.Exit(0)
	}

	fmt.Printf(" Calgo, a calendar program written in Go.  Looks for calgo.conf or calgo.ini.  Last altered %s, using %s.\n", lastCompiled, runtime.Version())
...
func findAndReadConfIni() ([]string, error) {
	...
	for _, token := range tokenslice {
		name := "*" + token.Str + "*" // make the name into *name* for the Excel pattern
	...
	//sort.Strings(docnames)  I decided that I don't want to sort this slice.
	//
	//if verboseFlag {
