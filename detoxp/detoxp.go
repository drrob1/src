...
------------------------------------------------------------------------------------------------------------------------------------------------------
   8 Feb 25 -- Now called detoxp, meaning using pflags
*/
...
func main() {
	...
	pflag.BoolVar(&noDotsFlag, "dots", false, "Enable removing excess dots from filenames.") // having 2 option names point to same variable makes them synonyms.
	pflag.BoolVarP(&noDotsFlag, "dot", "d", false, "Enable removing excess dots from filenames.")
	pflag.BoolVarP(&noWorkFlag, "no", "n", false, "No work is to be done.  Just show what would be done.")
	...
	if pflag.NArg() == 0 {
		...
		globPattern = strings.ToLower(pflag.Arg(0)) // first argument on command line.
	}

	startDirectory, _ := os.Getwd() // startDirectory is a string
	fmt.Println()
	fmt.Printf(" detoxp.go lastModified is %s, will use globbing pattern of %q and will start in %s.  noDotsFlag=%t, noWorkFlag=%t. \n",
