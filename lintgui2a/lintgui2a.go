...
------------------------------------------------------------------------------------------------------------------------------------------------------
   5 Apr 26 -- Now called lintGUI2a, and I intend to try a string data binding, which to me means a []string data binding, to define the list of files to choose which to scan for errors.
*/

const lastModified = "5 April 2026"
...
func main() {
	...
	a := app.NewWithID("lintgui2a")
	a.SetIcon(scheduleIconRes)
	s := fmt.Sprintf("Lint GUI 2a, last modified %s, compiled with %s", lastModified, runtime.Version())
	w := a.NewWindow(s)
	...
	filenamesList := binding.BindStringList(&filenames) // this creates the string binding

	selectFilenameList := widget.NewListWithData( // using a string binding to create the list
		filenamesList,
		...
		func(item binding.DataItem, o fyne.CanvasObject) {
			...
			filename.Bind(item.(binding.String))
		},
	)

	//selectFilenameList := widget.NewList( //using a list data type, without a string binding
	//	func() int {
	...
	monthsThresholdEntry.OnChanged = func(s string) {
		if s != "" {
			...
			err = filenamesList.Reload() // if this doesn't work, I could do something like filenamesList.Set(filenames).  Nope, not needed.
			if err != nil {
				dialog.ShowError(err, w)
			}
			//selectFilenameList.Refresh()  don't yet know if this is needed, as this is a binding after all.  Nope, not needed.
		}
	}
