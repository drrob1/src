...
//var green = color.NRGBA{R: 0, G: 100, B: 0, A: 255}    \ Turns out that these aren't used for the RichText widget.
...
func displayImage(img image.Image, fullImgName string, imgFmtName string) {
	imgName := filepath.Base(fullImgName)
	bounds := img.Bounds()
	imgHeight := bounds.Max.Y
	imgWidth := bounds.Max.X

	imgFI, err := os.Stat(fullImgName)
	if err != nil {
		fmt.Fprintln(os.Stderr, " Error from os.Stat of", fullImgName, "is", err)
		dialog.ShowError(err, globalW)
	}
	...
	if *verboseFlag {
		fmt.Println(title, "and cwd=", cwd, "and fullImgName", fullImgName)
	}
...
func rotateAndLoadTheImage(idx int, repeat int64) { // doesn't work yet in that the rotated image isn't being displayed correctly when hit 'r' key.
