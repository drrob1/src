...
var sliceOfResultStrings []string // based on an anonymous type.
...
func main() {
	...
	resultsChan = make(chan string, sliceSize)          // this is a buffered channel.
	sliceOfResultStrings = make([]string, 0, sliceSize) // this uses an anonymous type.
	go func() {                                         // start the receiving operation before the sending starts
		...
		for result := range resultsChan {
			sliceOfResultStrings = append(sliceOfResultStrings, result)
		}
		sort.Strings(sliceOfResultStrings) // the sort operation is now done here in the go routine, instead of the main function body.
	}()
	...
	// walk closure.  Only the last one is being used now.
	...
	close(resultsChan) // must close the channel so the matchChan for loop will end.  And I have to do this after all the work is done.
	...
	for _, s := range sliceOfResultStrings { //This is the only output that will be seen.
		fmt.Printf("%s", s) // remember that lineContents includes a \n at the end of each string.
	}
	...
	ctfmt.Printf(ct.Cyan, gotWin, "\n Elapsed %s to find and sort all of the matches, Elapsed since this all began is %s.\n\n", elapsed, outputElapsed)
} // end main
