package main // hightlight2.go from marker.go from highlight.go
...
------------------------------------------------------------------------------------------------------------------------------------------------------
  23 Dec 25 -- Now called highlight2, and I want to use an entry box to search for a file to open.  That is, I can bind the entry box to basenameSearchStr
*/

const lastModified = "25 Dec 25"
const width = 800
const height = 600
...
func main() {
	...
	var big image.Image
	var imgPath string
	var basenameSearchStrParam string

	pflag.StringVarP(&basenameSearchStrParam, "search", "s", "", "String to search base filename against.  If not specified, the program opens the first image file in the current directory.")
	pflag.Parse()

	if basenameSearchStrParam == "" { // this will allow either setting the search string as a command line argument or not.
		basenameSearchStrParam = pflag.Arg(1)
	}

	basenameSearchStr := binding.NewString()
	err := basenameSearchStr.Set(basenameSearchStrParam)
	if err != nil {
		panic(err)
	}
	...
	*/
	...
	openBtn2 := widget.NewButton("Open…2", func() {
		NewOpenFileDialogWithPrefix(w, entry.Text, []string{".png", ".jpg", ".jpeg", ".webp", ".gif", ".bmp"}, openFunc2)
	})
	...
	openBtnFunc := func() { // I want to specify starting directory 1st
		...
		openDialog.SetFilter(&nameFilterType{search: entry.Text}) // I hope if this is empty, then it matches everything.
