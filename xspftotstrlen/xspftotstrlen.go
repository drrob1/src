package main // xspfTotStrLen from shufv.go
...
/*
  ...
  27 Jan 24 -- Now called xspfTotStrLen.go.  I already wrote lv2 from launchv.  lv2 writes out an xspf file.  But I ran into a problem in sometimes the written xspf would not work.
                 I figured that there is a buffer in vlc that I'm exceeding and that chokes vlc so it doesn't work.  I am investigating this by finding out the total of the
                 location string lengths to see how high I can go.
...
const LastCompiled = "Jan 27, 2024"
...
func ProcessXMLfile(inputfile *bytes.Reader) {
	...
	// Need to skip the first 3 lines.
	for c := 0; c < 3; c++ {
		_, _ = readLine(inputfile)
	}
	...
	// ready to sum up the length of the location strings

	var totStrLen, maxStrLen, maxIdx int
	for i, trk := range TrackSlice {
		length := len(trk.location)
		totStrLen += length
		if length > maxStrLen {
			maxStrLen = length
			maxIdx = i
	...
	fmt.Printf(" There are %d tracks in the slice, with a total location string length of %d, taking %s for the arithmetic.\n", len(TrackSlice), totStrLen, elapsed)
	fmt.Printf(" The max string length is %d, index is %d, and the string is %s\n", maxStrLen, maxIdx, TrackSlice[maxIdx].location)
