...
/*
  ...
  22 Jun 22 -- Adding color to the output.
                 Now called feq2 and will use sha256.
*/
...
func main() {
	...
	if verboseFlag {
		fmt.Printf("\n feq2 File equal using sha256, last modified %s, compiled by %s\n\n", LastCompiled, runtime.Version())
	}
	...
	if sha1Str1 == sha1Str2 {
		if verboseFlag {
			ctfmt.Printf(ct.Green, winflag, " Sha256 values for %s and %s are equal.  Total elapsed time is %s.\n\n", filename1, filename2, time.Since(t0))
		} else {
			ctfmt.Printf(ct.Green, winflag, " sha256 values are equal.  Total elapsed time is %s.\n", time.Since(t0))
		}
	} else {
		ctfmt.Printf(ct.Red, winflag, " Sha256 for the files are not equal.\n %s = %x\n %s = %x\n Total elapsed time is %s.\n\n",
