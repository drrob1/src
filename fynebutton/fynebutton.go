...
func main() {
	...
	str := fmt.Sprintf("fynebutton example last modified %s, compiled using %s", LastModified, runtime.Version())
	...
	buttonfunc := func() {
		log.Println("tapped button")
	}

	btn := widget.NewButton("click me", buttonfunc)  // this way I can declare both but only use the Icon one.
	btn = widget.NewButtonWithIcon("click me", theme.HomeIcon(), buttonfunc)

	globalW.SetContent(btn)
