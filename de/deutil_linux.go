...
func getFileInfosFromCommandLine() []os.FileInfo {
	...
	if testFlag {
		fmt.Printf(" de: Entering getDirEntriesFromCommandLine.  flag.Nargs=%d, len(flag.Args)=%d\n", flag.NArg(), len(flag.Args()))
	}
	...
	if flag.NArg() == 0 {
		...
		if testFlag {
			fmt.Printf(" after call to myreaddir.  Len(dirEntries)=%d\n", len(fileInfos))
		}
