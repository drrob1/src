...
func main() {
	...
	if crc64ECMAval1 == crc64ECMAval2 {
		if verboseFlag {
			ctfmt.Printf(ct.Green, winflag, " crc64ECMA values for %s and %s are equal.  Total elapsed time is %s.\n\n", filename1, filename2, time.Since(t0))
		} else {
			ctfmt.Printf(ct.Green, winflag, " crc64 ECMA values are equal.  Total elapsed time is %s.\n", time.Since(t0))
		}
	} else {
		ctfmt.Printf(ct.Red, winflag, " crc64 ECMA for the files are not equal.\n %s = %x\n %s = %x\n Total elapsed time is %s.\n\n",
