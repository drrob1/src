...
//	fmt.Printf("%T\n",i);
	...
	switch i.(type) {
	  case string:  fmt.Println("It's a string",i.(string)); // this asserts the type
	  case uint32:  fmt.Println("It's a uint32",i.(uint32)); // this asserts the type
	  case int: 
		I := i.(int);
	...
	switch v := i.(type) {
	  ...
	  case uint32:  fmt.Println("It's a uint32",v);
	  default:  fmt.Printf("IDK %v\n",v);
	}
}
