...
/*
This hashtable is based on open addressing, which is also called closed hashing.  This is the 3rd hashing exercise.  Here we use a key's hashed value to directly
...
This project will add delete.  The complexity of delete is that this entry needs to be non-nil because other elements may depend on this being non-nil for their probe sequence to succeed.
...
func (hashTable *LinearProbingHashTable) find(name string) (int, int) {
	...
	for {
		jumps++
		if hashTable.employees[i] == nil { // key not found, but came to an empty slot so here's where a new key may be added, but if there's a deleted Index, return that.
			if deletedIndex != -1 {
				return deletedIndex, jumps
			}
			return i, jumps
		}
		if hashTable.employees[i].deleted { // this will return the first deleted entry in a probe sequence if the key is not found.
			if deletedIndex == -1 {
				deletedIndex = i
			}
		} else if hashTable.employees[i].name == name { // found key and it's not been deleted.
			return i, jumps
		}
		i++
