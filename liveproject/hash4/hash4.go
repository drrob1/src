...
/*
This hashtable is based on open addressing, which is also called closed hashing.  This is the 4th hashing exercise.  It will introduce quadratic probing, designed to limit primary clustering.
Primary clustering slows down the pgm.
...
func NewQuadraticProbingHashTable(capacity int) *QuadraticProbingHashTable {
	LPHT := QuadraticProbingHashTable{ // LPHT = Linear Probing Hash Table, of course.
		capacity:  capacity,
...
func (hashTable *QuadraticProbingHashTable) find(name string) (int, int) {
	...
	for i := 0; i < hashTable.capacity; i++ {
		idx := (i*i + h) % hashTable.capacity // now it's a quadratic.  Note that the first time thru, i = 0, so i^2 is also zero.
		jumps++
...
func (hashTable *QuadraticProbingHashTable) set(name, phone string) {
...
func (hashTable *QuadraticProbingHashTable) probe(name string) int {
	...
	for i := 0; i < hashTable.capacity; i++ {
		index := (hash + i*i) % hashTable.capacity // now has quadratic probing.  i = 0 first time thru.
