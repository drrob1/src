...
/*
  REVISION HISTORY
  -------- -------
   3 Nov 20 -- Added the deadline function, as it's timing out at work.
 */


// This program implements a trivial NTP client over UDP.
// It uses NTP version 3 data packet format which is a 48-byte long datagram for both request and response.
// Usage:  ntpc -e <host endpoint>
func main() {
	var host string
	...
	// set up the deadline value
	timenow := time.Now()
	timeout := timenow.Add(10*time.Second)
	...
	// Once connection is established, the code pattern is the same as in the other impl.
