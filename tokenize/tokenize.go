...
var  CH1,CH2 byte;
  ...
  switch (CH1) {
    case LTSIGN : OpCode = 1;
    case GTSIGN : OpCode = 3;
    case EQUALSIGN : OpCode = 5;
    case  PLUSSIGN  : OpCode = 8;
    case  MINUSSIGN : OpCode = 10;
    case  POUNDSIGN : OpCode = 7;
    case  MULTSIGN  : OpCode = 12;
    case  DIVSIGN   : OpCode = 14;
    case  EXPSIGN   : OpCode = 16;
    case  PERCNT    : OpCode = 22;
...
//          TOKEN on this line.
    ...
    if QUOFLG && (CHAR.Ch != NullChar) { CHAR.State = ALLELSE }
      ...
      case DELIM : // token.state
        switch CHAR.State {
          case DELIM : // NULL char is a special delimiter because it will
                   ...
                   if CHAR.Ch == NullChar { break ExitForLoop } // goto ExitLoop }
          ...
          case DGT :    // OP -> DGT means it may be a sign character for a number token.  If not, have 1 char operator
            ...
            if (LastChar == '+') || (LastChar == '-') {
              if len(tokenByteSlice) == 1 {
                if tokenByteSlice[0] == '-' { NEGATV = true; }
                TOKEN.State = DGT;
...
//                  TOKEN.Str = TOKEN.Str[:upperbound]; // del last char of the token which is the sign character
      ...
      case DGT : // tokenstate
        switch CHAR.State {
          case DELIM : break ExitForLoop;    //goto ExitLoop;
          case OP :    // DGT -> OP
                    UNGETCHR();
                    break ExitForLoop;  //goto ExitLoop;
...
//                                                                          TOKEN.Str = strings.ToUpper(TOKEN.Str);
  ...
  if (TOKEN.State == DGT) && NEGATV { TOKEN.Isum = -TOKEN.Isum; }
...
//                        token on this line.
    ...
    switch (CHAR.State) {
      case DELIM :                    // Ignore leading delims
                   if Len > 0 { break ExitLoop }  //goto ExitLoop; }
      case OP :
...
// the only TOKENIZE procedure that uses it.
  ...
  for {
    Char,EOL = GETCHR();   // the Cap function is not here anymore.
    if EOL { break } //  No-go.  Need to change this to idiomatic Go, but after debugging the main GetTkn and UnGetTkn rtns.
    tokenByteSlice = append(tokenByteSlice,Char.Ch);
